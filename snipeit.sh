...
#/ Usage: snipeit.sh [-vh]
#/
#/ Install Parque Seguro open source asset management.
#/
#/ OPTIONS:
...
#                                                    #
#         Updated Parque Seguro Install Script            #
...
create_user () {
  echo "* Creating Parque Seguro user."

  if [[ "$distro" == "Ubuntu" ]] || [[ "$distro" == "Debian" ]] || [[ "$distro" == "Raspbian" ]] ; then
    /usr/sbin/adduser --quiet --disabled-password --gecos 'Parque Seguro User' "$APP_USER"
...
install_snipeit () {
  ...
  echo -e "\n\n* Cloning Parque Seguro from github to the web directory."
  log "git clone https://github.com/snipe/Parque Seguro $APP_PATH" & pid=$!
  progress
  pushd $APP_PATH
  git checkout master
  popd
  ...
  sed -i '1 i\#Created By Parque Seguro Installer' "$APP_PATH/.env"
...
echo ""
echo "  Welcome to Parque Seguro Inventory Installer for CentOS, Rocky, Debian, and Ubuntu!"
echo ""
...
echo ""
echo "  ***Open http://$fqdn to login to Parque Seguro.***"
echo ""
echo ""
