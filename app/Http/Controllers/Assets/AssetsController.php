    ...
    public function create(Request $request) : View
{
    ...
    // Definindo a variável imagePath como null no create, já que não há imagem ainda
    $imagePath = null;
    ...
    $view = view('hardware/edit')
        ...
        ->with('imagePath', $imagePath);  // Passando imagePath como null
...
$asset->status = 'Checked Out'; // Aqui define o estado como "Checked Out"
        $asset->status_id = 25; // ID correspondente ao estado "Checked Out"
    ...
    public function edit($assetId = null) : View | RedirectResponse
{
    ...
    // Gerando a URL segura para a imagem
    // Garantir que estamos usando o nome do arquivo sem a base URL duplicada
    $imagePath = $item->image ? route('assets.foto', ['filename' => basename($item->image)]) : null;

    return view('hardware/edit', compact('item', 'imagePath'))
        ->with('statuslabel_list', Helper::statusLabelList())
        ->with('statuslabel_types', Helper::statusTypeList());
}
    ...
    public function show($assetId = null) : View | RedirectResponse
{
    ...
    // ✅ carregar notas do EE
    $notasEe = NotaEe::where('asset_id', $assetId)
        ->with('responsavel')
    ...
    if ($asset) {
        ...
        $use_currency = $asset->location->currency ?? $settings->default_currency ?? trans('general.currency');
        ...
        return view('hardware/view', compact('asset', 'qr_code', 'settings', 'notasEe'))
