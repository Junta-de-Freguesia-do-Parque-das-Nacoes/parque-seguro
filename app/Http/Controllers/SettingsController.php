...
class SettingsController extends Controller
{
    ...
    public function saveLabelTemplate(Request $request)
    {
        // Isto já está garantido pelo middleware 'auth', mas se quiseres, podes confirmar:
        if (!auth()->check()) {
            return response()->json(['error' => 'Não autenticado'], 403);
        }
    
        $setting = \App\Models\Setting::first();
        $setting->label2_template = $request->label2_template;
        $setting->save();
    ...
    public function postBranding(ImageUploadRequest $request) : RedirectResponse
    {
        ...
        if (!config('app.lock_passwords')) {
            ...
            $setting->site_name = $request->input('site_name', 'Parque Seguro');
