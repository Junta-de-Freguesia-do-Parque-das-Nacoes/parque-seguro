    ...
    public function index(Request $request, $action = null, $upcoming_status = null) : JsonResponse | array
    {
        ...
        $assets = Asset::select('assets.*')
            ->with('location', 'assetstatus', 'company', 'defaultLoc','assignedTo',
                ...
                if ($request->filled('sem_ee') && $request->input('sem_ee') === 'sem') {
                    $assets->whereDoesntHave('responsaveis', function ($query) {
...
if ((! is_null($filter)) && (count($filter)) > 0) {
    ...
    $assets->where(function ($query) use ($searchTerm, $all_custom_fields) {
        // Procurar o termo exato no nome completo ou nome_apelido
        $query->where('assets.name', 'LIKE', "%{$searchTerm}%")
              ->orWhere('assets.nome_apelido', 'LIKE', "%{$searchTerm}%")  // Inclui pesquisa no nome_apelido
              ->orWhere('assets.asset_tag', 'LIKE', "%{$searchTerm}%")  
              ->orWhere('assets.serial', 'LIKE', "%{$searchTerm}%");

        // Incluir pesquisa nos campos personalizados
        foreach ($all_custom_fields as $field) {
            if (!empty($field->db_column_name())) {
                $query->orWhere($field->db_column_name(), 'LIKE', "%{$searchTerm}%");
            }
        }
    });
}
