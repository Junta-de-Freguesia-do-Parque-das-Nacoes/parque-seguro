...
class ActionlogsTransformer
{
    ...
    public function transformActionlog (Actionlog $actionlog, $settings = null)
    {
            ...
            if ($meta_array) {
                ...
                foreach ($meta_array as $fieldname => $fieldata) {
                    ...
                    if (is_object($fieldata)) {
                        $clean_meta[$fieldname]['old'] = $this->clean_field($fieldata->old ?? null);
                        $clean_meta[$fieldname]['new'] = $this->clean_field($fieldata->new ?? null);
                    } elseif (is_scalar($fieldata)) {
                        $clean_meta[$fieldname]['old'] = e($fieldata);
                        $clean_meta[$fieldname]['new'] = e($fieldata);
                    } else {
                        Log::warning("⚠️ Dados inesperados no log_meta", [
                            'fieldname' => $fieldname,
                            'valor' => $fieldata,
                            'action_log_id' => $actionlog->id,
                        ]);
                        $clean_meta[$fieldname]['old'] = 'Dados inválidos';
                        $clean_meta[$fieldname]['new'] = 'Dados inválidos';
                    }
                
                    // Verifica se é um campo personalizado encriptado
                    if (str_starts_with($fieldname, '_snipeit_')) {
                        foreach ($custom_fields as $custom_field) {
                            if ($custom_field->db_column == $fieldname && $custom_field->field_encrypted == '1') {
        ...
        $array = [
            ...
            'action_source_label' => $actionlog->present()->actionSourceLabel(), // NOVO
