...
function url_get_contents ($Url) {
    if (!function_exists('curl_init')){
        die("cURL is not installed!\nThis is required for Parque Seguro as well as the upgrade script, so you will need to fix this before continuing.\nAborting upgrade...\n");
    }
    $ch = curl_init();
...
echo "--------------------------------------------------------\n";
echo "WELCOME TO THE Parque Seguro UPGRADER! \n";
echo "--------------------------------------------------------\n\n";
...
// Fetching most current upgrade requirements from github. Read more here: https://github.com/snipe/Parque Seguro/pull/14127
$remote_requirements_file = "https://raw.githubusercontent.com/snipe/Parque Seguro/$branch/.upgrade_requirements.json";
...
if(!$skip_php_checks){
    ...
    if ((version_compare(phpversion(), $php_min_works, '>=')) && (version_compare(phpversion(), $php_max_wontwork, '<'))) {
        ...
        echo "This version of PHP (".phpversion().") is NOT compatible with Parque Seguro.\n";
        echo "Parque Seguro requires PHP versions between ".$php_min_works." and ".$php_max_wontwork.".\n";
...
if ((strpos('git version', $git_version)) === false) {
    ...
    echo "Please note that this script will not download the latest Parque Seguro \n";
...
echo "your upgraded Parque Seguro!\n";
echo "--------------------------------------------------------\n\n";
